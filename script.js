...
editSaveBtn.addEventListener("click", async function () {
  ...
  await showAlert(
    `Expense updated: ${name} (${formatCurrency(amount)})`,
    "Updated!",
    "success",
  );
});
...
const remainingBalanceDisplay = document.getElementById(
...
function updatePayPeriodDisplay(startDate, endDate) {
  ...
  currentPayPeriodDisplay.textContent = `Pay Period: ${formatDate(startDate)} - ${formatDate(endDate)}`;
...
function renderExpensesList() {
  ...
  if (currentBudgetData.expenses.length === 0) {
    // Show empty message
    expensesList.innerHTML =
      '<p class="empty-message">No expenses yet. Add one above!</p>';
  ...
  Object.keys(groupedExpenses).forEach((dateKey) => {
    ...
    groupedExpenses[dateKey].forEach((expense, dateIndex) => {
      ...
      const originalIndex = currentBudgetData.expenses.findIndex(
        (e) =>
          e.name === expense.name &&
...
netPayBtn.addEventListener("click", async function () {
  ...
  await showAlert(
    `Net pay set to ${formatCurrency(amount)}`,
    "Success!",
    "success",
  );
});
...
setSavingsBtn.addEventListener("click", async function () {
  ...
  await showAlert(
    `Savings target set to ${formatCurrency(amount)}`,
    "Savings Set!",
    "success",
  );
});
...
addExpenseBtn.addEventListener("click", async function () {
  ...
  await showAlert(
    `${name} added (${formatCurrency(amount)}) on ${formatDateDisplay(date)}`,
    "Expense Added",
    "success",
  );
});
...
newBudgetBtn.addEventListener("click", async function () {
  ...
  await showAlert(
    "New budget started! Previous data saved.",
    "Budget Reset",
    "success",
  );
});
